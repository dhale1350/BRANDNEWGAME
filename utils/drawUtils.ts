...
export const TOOL_PALETTES = {
  ...
  iron: { base: '#eceff1', dark: '#b0bec5', light: '#ffffff', outline: '#546e7a' }, // Tweaked outline for better contrast
...
export const drawWall = (ctx: CanvasRenderingContext2D, x: number, y: number, type: WallType, wx: number, wy: number) => {
    ctx.save();
    ctx.translate(x, y);
    ctx.fillStyle = WALL_COLORS[type];
...
export const drawBlock = (ctx: CanvasRenderingContext2D, x: number, y: number, type: BlockType, wx: number, wy: number) => {
    ...
    if ([BlockType.STONE, BlockType.BRICK, BlockType.COAL, BlockType.IRON, BlockType.GOLD, BlockType.DIAMOND, BlockType.WOOD].includes(type)) {
...
export const drawTool = (
    ...
    // Draw pixel relative to local origin
    ...
    // Sprites are drawn facing UP (negative Y)
    // Origin (0,0) is approximately the center of the handle grip
    ...
    if (type === ToolType.SWORD) {
        ...
        // Curved Head (Arc)
        // Left Side
        dp(-1, -4, p.base); dp(-2, -3, p.base); dp(-3, -2, p.dark); 
        dp(-4, -1, p.outline); // Sharp tip left
        
        // Right Side
        dp(1, -4, p.base); dp(2, -3, p.base); dp(3, -2, p.dark);
        dp(4, -1, p.outline); // Sharp tip right
        ...
        // Back of axe (small counterweight)
        dp(-1, -3, p.dark);
    }
        ...
        dp(0, -8, p.light); dp(-1, -8, p.light); dp(1, -8, p.light); // Tip edge
    }
...
export const drawArmor = (ctx: CanvasRenderingContext2D, type: ArmorType, tier: number, x: number, y: number, scale: number = 1) => {
    ...
    if (type === ArmorType.HELMET) {
        ...
        // Face Guard / Visor
        if (tier >= 4) { // Diamond (Full helm with faceplate)
            ...
            dp(5, -6, p.light); dp(5, -7, p.light); dp(6, -8, p.light);
        } else { // Iron (Open faced or T-slit)
            dp(-3, -3, p.dark); dp(3, -3, p.dark); // Shadows near eyes
            dp(-4, -1, p.outline); dp(4, -1, p.outline); // Cheek guards edge
        ...
        for(let y=-4; y<=1; y++) {
            for(let x=-4; x<=4; x++) {
                if (Math.abs(x) > 3 && y > -2) continue; // Taper waist
                dp(x, y, p.base);
            }
        }
        
        // Chest Definition (Pecs/Detail)
        dp(-2, -3, p.light); dp(2, -3, p.light);
        dp(0, -3, p.dark); dp(0, -2, p.dark);
        
        // Abs/Belly
        dp(-1, 0, p.dark); dp(1, 0, p.dark);
        dp(0, 1, p.outline); // Belt buckle area placeholder
    }
    else if (type === ArmorType.LEGGINGS) {
        // Waistband
        for(let i=-3; i<=3; i++) dp(i, -4, p.dark);
        dp(0, -4, p.light); // Buckle
        
        // Crotch Area
        dp(0, -3, p.base); dp(-1, -3, p.base); dp(1, -3, p.base);
        dp(0, -2, p.dark);
        ...
        dp(-2, 0, p.dark); dp(-3, 0, p.dark); // Knee shadow/pad
        dp(-2, 1, p.base); dp(-3, 1, p.base); // Greave
