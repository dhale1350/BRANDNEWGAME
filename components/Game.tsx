...
export const Game: React.FC<GameProps> = ({ 
  ...
  const updateInteraction = useCallback((now: number, dt: number) => {
      ...
      if (effectiveIsMobile) {
          ...
          if (gp) { 
              ...
              // Calculate aim position even if stick is neutral (default to front)
              if (Math.abs(ax) > 0.1 || Math.abs(ay) > 0.1) { 
                  cursorX = p.x + p.width/2 + ax * 150; cursorY = p.y + p.height/2 + ay * 150; 
              } else {
                  const dir = p.facing === 'right' ? 1 : -1;
                  cursorX = p.x + p.width/2 + dir * 60; 
                  cursorY = p.y + p.height/2;
              }
  ...
  const renderLoop = (time: number) => {
    ...
    // Updated reticle drawing to include gamepad
    if ((effectiveIsMobile || inputRef.current.isGamepadActive) && targetReticleRef.current.valid) { 
